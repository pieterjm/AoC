...
#    print(key)
    ...
    bSolved = True
    for a,r,c in [['A',2,3],['A',3,3],['A',4,3],['A',5,3],['B',2,5],['B',3,5],['B',4,5],['B',5,5],['C',2,7],['C',3,7],['C',4,7],['C',5,7],['D',2,9],['D',3,9],['D',4,9],['D',5,9]]:
        if maze[r][c] != a:
            bSolved = False
    if bSolved == True:
    ...
    for a,c in caves:
        ...
        if len(to) > 0:
            if maze[2][c] != '.' and  (maze[2][c] != a or maze[3][c] != a or maze[4][c] != a or maze[5] != a):
                for t in to:
                    m.append([[2,c],t])
            if maze[2][c] == '.' and maze[3][c] != '.' and (maze[3][c] != a or maze[4][c] != a or maze[5][c] != a):
                for t in to:
                    m.append([[3,c],t])
            if maze[2][c] == '.' and maze[3][c] == '.' and maze[4][c] != '.' and (maze[4][c] != a or maze[5][c] != a):
                for t in to:
                    m.append([[4,c],t])
            if maze[2][c] == '.' and maze[3][c] == '.' and maze[4][c] == '.' and maze[5][c] != a and maze[5][c] != '.':
                for t in to:
                    m.append([[5,c],t])                    
        ...
        for t in [c-1,c+1]:
            if maze[1][t] == a:
                if maze[5][c] == a and maze[4][c] == a and maze[3][c] == a and maze[2][c] == '.':
                    m.append([[1,t],[2,c]])
                if maze[5][c] == a and maze[4][c] == a and maze[3][c] == '.' and maze[2][c] == '.':
                    m.append([[1,t],[3,c]])
                if maze[5][c] == a and maze[4][c] == '.' and maze[3][c] == '.' and maze[2][c] == '.':
                    m.append([[1,t],[4,c]])
                if maze[5][c] == '.' and maze[4][c] == '.' and maze[3][c] == '.' and maze[2][c] == '.':
                    m.append([[1,t],[5,c]])
